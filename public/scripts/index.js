  ...
  if (navigator.onLine) {
    (async function cacheRequests() {
      await caches.open('CACHE-7').then(function (cache) {
        return cache.addAll([`/letters/${localStorage.from}`, `/mypen/${localStorage.secret}`]);
      });
    }());
  }
...
function renderLetters(letters) {
  ...
  if (letters.length !== 0) {
    ...
    date.innerHTML = new Date(letters[last].timeStamp).toLocaleDateString('en-us');
    ...
    next.addEventListener('click', () => {
      ...
      if (last === count) {
        ...
        date.innerHTML = new Date(letters[count].timeStamp).toLocaleDateString('en-us');
      }
    });
